...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplication.LaunchOptionsKey: Any]?) -> Bool {
        // this will find the documents folder in our sandbox and then look for a file called saveData.plist and overwrite the generic model
        do {
            ...
            try arr.forEach {
                if($0.lastPathComponent == "saveData.plist") {
                    ...
                    // This saves our data from the saved model to the new and current model
                    ...
                    self.ourModel.clickedTiles = model.clickedTiles
                    self.ourModel.bombs = model.bombs
                    self.ourModel.currentTimePlayed = model.currentTimePlayed
                    self.ourModel.gameState = model.gameState
                    self.ourModel.firstClick = model.firstClick
                }
            }
        } catch {
